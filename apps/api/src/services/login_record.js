...
const Login_recordInterfaces = require('../interfaces/login_record')
const Login_recordContract = require('../contracts/login_record')
...
class Login_recordService {
	 ...
	 * @private
	 */
	login_recordInterface

	/**
	 * @private
	 */
	login_recordContract
	...
	constructor() {
		...
		this.login_recordInterface = Login_recordInterfaces.getInstance()
		this.login_recordContract = Login_recordContract.getInstance()
	...
	static getInstance() {
		if (!this.instance) this.instance = new Login_recordService()
	...
	async add(config = {}) {
		...
		const data = Array.isArray(unflattenedBody)
			? unflattenedBody.map(el => this.login_recordInterface.getCreateInterface().parse(el))
			: this.login_recordInterface.getCreateInterface().parse(unflattenedBody)
		
		let login_record = await this.repository.add('login_record', { data, options })

		// 2. Handle files if present
		if (files && files.length) {
			const login_recordId = Array.isArray(login_record) ? login_record[0]._id : login_record._id
			...
			try {
				for (const file of files) {
					...
					const newFilename = `login_record-${crypto.randomUUID()}-${originalName}`
				...
				if (Object.keys(updates).length > 0) {
					login_record = await this.repository.update('login_record', { id: login_recordId, data: updates, options })
				}
			} catch (err) {
		...
		return this.applayContract(login_record)
	}
	...
	async findOne(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		const query = this.login_recordInterface.getQueryInterface().parse({ _id: id, ...config.query?.query })
		if (config.query?.virtuals) virtuals = this.login_recordInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.login_recordInterface.getRelationsInterface().parse(config.query.relations)
		const options = config.options || {}

		const result = await this.repository.list('login_record', { query, virtuals, relations, options })
		const login_record = result.records[0]
		if (!login_record) throw new BadRequestError('Login record not found.')
	...
	async list(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		if (config.query?.query) query = this.login_recordInterface.getQueryInterface().parse(config.query.query)
		if (config.query?.virtuals) virtuals = this.login_recordInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.login_recordInterface.getRelationsInterface().parse(config.query.relations)
		...
		const login_record_list = await this.repository.list('login_record', { query, virtuals, relations, size, page, sort, options })
		login_record_list.records = this.applayContract(login_record_list.records)
		return login_record_list
	}
	
	async update(config = {}) {
		const { body, id, files = [], options = {} } = config
		const data = this._unflatten(body)
		const existingLogin_record = await this.findOne({ id })
		const destinationPath = envVariables.API_UPLOAD_PATH || path.join(process.cwd(), 'api-uploads')

		// 1. Initial update with JSON data
		const payload = this.login_recordInterface.getUpdateInterface().parse(data)
		let login_record = await this.repository.update('login_record', { id, data: payload, options })
		
		const existingObj = existingLogin_record.toObject ? existingLogin_record.toObject() : existingLogin_record;
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					login_record = await await this.repository.update('login_record', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async replace(config = {}) {
		...
		const payload = this.login_recordInterface.getUpdateInterface().parse(data)
		let login_record = await await this.repository.replace('login_record', { id, data: payload, options })
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					login_record = await this.repository.update('login_record', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async remove(config = {}) {
		...
		const repositoryResponse = await await this.repository.remove('login_record', { id, options })
	...
	applayContract(payload) {
		const contract = this.login_recordContract.getContract()
...
module.exports = Login_recordService