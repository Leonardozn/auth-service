...
class SessionService {
	...
	constructor() {
		...
		this.sessionInterface = SessionInterfaces.getInstance()
		this.sessionContract = SessionContract.getInstance()
	...
	async add(config = {}) {
		...
		const data = Array.isArray(unflattenedBody)
			...
			: this.sessionInterface.getCreateInterface().parse(unflattenedBody)
		...
		if (files && files.length) {
			...
			try {
				for (const file of files) {
					...
					const newFilename = `session-${crypto.randomUUID()}-${originalName}`
				...
				if (Object.keys(updates).length > 0) {
					session = await this.repository.update('session', { id: sessionId, data: updates, options })
				}
			} catch (err) {
	...
	async findOne(config = {}) {
		...
		if (config.query?.virtuals) virtuals = this.sessionInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.sessionInterface.getRelationsInterface().parse(config.query.relations)
		const options = config.options || {}

		const result = await this.repository.list('session', { query, virtuals, relations, options })
	...
	async list(config = {}) {
		...
		let virtuals = {}
		let relations = {}
		if (config.query?.query) query = this.sessionInterface.getQueryInterface().parse(config.query.query)
		if (config.query?.virtuals) virtuals = this.sessionInterface.getVirtualsInterface().parse(config.query.virtuals)
		if (config.query?.relations) relations = this.sessionInterface.getRelationsInterface().parse(config.query.relations)
		...
		const session_list = await this.repository.list('session', { query, virtuals, relations, size, page, sort, options })
		session_list.records = this.applayContract(session_list.records)
		return session_list
	}
	...
	async update(config = {}) {
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					session = await await this.repository.update('session', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async replace(config = {}) {
		...
		if (files && files.length) {
			...
			try {
				...
				if (Object.keys(updates).length > 0) {
					session = await this.repository.update('session', { id, data: updates, options })
				}
			} catch (err) {
				for (const url of savedFiles) {
					await this.storageProvider.deleteFile(url, destinationPath).catch(() => {})
	...
	async remove(config = {}) {
		...
		const repositoryResponse = await await this.repository.remove('session', { id, options })
	...
	applayContract(payload) {
		const contract = this.sessionContract.getContract()
